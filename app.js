...
function showMovies(data) {
    ...
    data.forEach(movie => {
        ...
        movieEl.innerHTML = `
            ...
            <h3>Overview</h3> 
            <p> ${overview}</p>
            <button class="trailer" id="trailer-${id}">Movie Trailer</button>
            <button class="know-more" id="movie-click-action-${id}">Know More</button>
            </div>`

        main.appendChild(movieEl);
        document.getElementById(`trailer-${id}`).addEventListener('click', () => {
            console.log(id)
            openNav(movie)
        })
    
        document.getElementById(`movie-click-action-${id}`).addEventListener('click', ()=>{
            openNav2(movie)
        })
    })
}
